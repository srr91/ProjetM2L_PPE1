...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $other_id > 0) {
    ...
    if ($contenu === '') {
        ...
        if ($stmt->execute([$user_id, $other_id, $contenu])) {
            header('Location: /old/espace-coach/messages.php?with=' . $other_id);
            exit();
        }
            ...
            <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                ...
                <?php else: ?>
                    <?php foreach ($contacts as $c): ?>
                        ...
                        ?>
                        <a href="/old/espace-coach/messages.php?with=<?= (int)$c['id'] ?>" style="text-decoration: none; color: inherit;">
            ...
            <div id="chat" class="sc-chat">
                ...
                <?php if ($other_id === 0): ?>
                    <div style="color: var(--gray); text-align: center; padding: 2rem;">
                        Choisissez un sportif pour r̩pondre.
                    </div>
                <?php else: ?>
                            ...
                            <div class="sc-msg-row <?= $mine ? 'sc-msg-row--mine' : 'sc-msg-row--theirs' ?>">
                                <?php if (!$mine): ?>
                                    <?php if (!empty($sportif['photo_profil']) && file_exists(__DIR__ . '/../telechargements/profils/' . $sportif['photo_profil'])): ?>
                                        <img class="sc-avatar" src="/telechargements/profils/<?= htmlspecialchars($sportif['photo_profil']) ?>" alt="" />
                                    <?php else: ?>
            ...
            <div class="sc-composer">
                ...
                <?php else: ?>
                    <div style="color: var(--gray); text-align: center; padding: 1rem;">
                        S̩lectionnez un sportif pour ̩crire.
                    </div>
                <?php endif; ?>
            </div>
        </div>
...
(function() {
    ...
    const scTheirAvatarUrl = <?= json_encode(!empty($sportif['photo_profil']) ? ('/telechargements/profils/' . $sportif['photo_profil']) : '') ?>;
    ...
    async function markRead(id) {
        ...
        try {
            ...
            await fetch('/old/api/messages-lus.php', { method: 'POST', body: fd, credentials: 'same-origin' });
        } catch (e) {}
    }
