...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $other_id > 0) {
    ...
    if ($contenu === '') {
        ...
        if ($stmt->execute([$user_id, $other_id, $contenu])) {
            header('Location: /SportConnect/old/espace-coach/messages.php?with=' . $other_id);
            exit();
        }
            ...
            <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                ...
                <?php else: ?>
                    <?php foreach ($contacts as $c): ?>
                        ...
                        ?>
                        <a href="/SportConnect/old/espace-coach/messages.php?with=<?= (int)$c['id'] ?>" style="text-decoration: none; color: inherit;">
