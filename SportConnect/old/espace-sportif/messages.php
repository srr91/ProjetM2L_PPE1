...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $other_id > 0) {
    ...
    if ($contenu === '') {
        ...
        if ($stmt->execute([$user_id, $other_id, $contenu])) {
            header('Location: /sportif/messages.php?with=' . $other_id);
            exit();
        }
    ...
    <div style="display: grid; grid-template-columns: 320px 1fr; gap: 1.5rem; align-items: start;">
            ...
            <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                ...
                <?php else: ?>
                    <?php foreach ($contacts as $c): ?>
                        ...
                        ?>
                        <a href="/sportif/messages.php?with=<?= (int)$c['id'] ?>"
        ...
        <div class="card" style="min-height: 520px; display: flex; flex-direction: column;">
            <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; gap: 1rem;">
                ...
                <?php if ($coach): ?>
                    <a class="btn btn-outline" href="/pages/profil-coach.php?id=<?= (int)$coach['id'] ?>">Voir profil</a>
                <?php endif; ?>
            </div>
            ...
            <div class="sc-composer">
                ...
                <?php else: ?>
                    <div style="color: var(--gray); text-align: center; padding: 1rem;">
                        Sélectionnez un coach pour écrire.
                    </div>
                <?php endif; ?>
            </div>
        </div>
...
(function() {
    ...
    async function markRead(id) {
        ...
        try {
            ...
            const res = await fetch('/api/messages-lus.php', { method: 'POST', body: fd, credentials: 'same-origin' });
            if (!res.ok) return;
    ...
    async function pollChat() {
        ...
        inFlight = true;
        try {
            const res = await fetch('/api/actualisation-messages.php?last_id=' + encodeURIComponent(String(lastId)), { credentials: 'same-origin', cache: 'no-store' });
            if (!res.ok) return;
            const data = await res.json();
