...
$other_id = $coach_id ?: $selected_id;

// Liste des conversations (coachs) du sportif
$stmt = $conn->prepare("
    SELECT 
        CASE 
...
if (!empty($conversations)) {
    ...
    $stmt = $conn->prepare("SELECT id, prenom, nom, role, photo_profil FROM utilisateurs WHERE id IN ($placeholders) AND role = 'coach' AND actif = 1");
    $stmt->execute($ids);
    $rows = $stmt->fetchAll();
...
if ($other_id > 0) {
    $stmt = $conn->prepare("
        SELECT u.id, u.prenom, u.nom, u.role, u.photo_profil,
               pc.specialite, pc.localisation, pc.valide
        FROM utilisateurs u
        LEFT JOIN profils_coachs pc ON pc.user_id = u.id
    ...
    <div style="display: grid; grid-template-columns: 320px 1fr; gap: 1.5rem; align-items: start;">
        <div class="card">
            ...
            <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                <a href="../coachs.php" class="btn btn-outline" style="width: 100%;">Trouver un coach</a>
                ...
                <?php else: ?>
                    <?php foreach ($contacts as $c): ?>
                        ...
                        ?>
                        <a href="/old/espace-sportif/messages.php?with=<?= (int)$c['id'] ?>"
        ...
        <div class="card" style="min-height: 520px; display: flex; flex-direction: column;">
                ...
                <div>
                    ...
                    <?php if ($coach): ?>
                        <div style="color: var(--gray);">
                            <?= htmlspecialchars($coach['specialite'] ?? 'Coach') ?>
                        </div>
                    <?php endif; ?>
                </div>

                <?php if ($coach): ?>
                    <a class="btn btn-outline" href="/old/pages/profil-coach.php?id=<?= (int)$coach['id'] ?>">Voir profil</a>
                <?php endif; ?>
            </div>
            ...
            <div id="chat" class="sc-chat">
                <button type="button" id="scNewMsg" class="sc-new-msg">Nouveau message â†“</button>
                <?php if ($other_id === 0): ?>
                ...
                <?php else: ?>
                    <?php if (empty($messages)): ?>
                        <div style="color: var(--gray); text-align: center; padding: 2rem;">
                            Aucun message. Ã‰crivez le premier.
                        </div>
                    <?php else: ?>
                        <?php foreach ($messages as $m): ?>
                            <?php $mine = ((int)$m['sender_id'] === $user_id); ?>
                            <?php
                                $otherName = trim(($coach['prenom'] ?? '') . ' ' . ($coach['nom'] ?? ''));
                            ...
                            <div class="sc-msg-row <?= $mine ? 'sc-msg-row--mine' : 'sc-msg-row--theirs' ?>">
                                <?php if (!$mine): ?>
                                    <?php if (!empty($coach['photo_profil']) && file_exists(__DIR__ . '/../telechargements/profils/' . $coach['photo_profil'])): ?>
                                        <img class="sc-avatar" src="/telechargements/profils/<?= htmlspecialchars($coach['photo_profil']) ?>" alt="" />
                                    <?php else: ?>
                                ...
                                <div class="sc-msg-col">
                                    ...
                                    <div class="sc-meta">
                                        ...
                                        <?php if ($mine): ?>
                                            ...
                                            <span class="sc-status <?= $isRead ? 'sc-status--read' : 'sc-status--sent' ?>"><?= $isRead ? 'âœ“âœ“' : 'âœ“' ?></span>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                ...
                                <?php if ($mine): ?>
                                    <?php if (!empty($me['photo_profil']) && file_exists(__DIR__ . '/../telechargements/profils/' . $me['photo_profil'])): ?>
                                        <img class="sc-avatar" src="/telechargements/profils/<?= htmlspecialchars($me['photo_profil']) ?>" alt="" />
                                    <?php else: ?>
            ...
            <div class="sc-composer">
                <?php if ($other_id > 0): ?>
                    <form method="POST" id="scComposerForm">
                        ...
                        <button type="submit" id="scComposerSend" class="sc-send" aria-label="Envoyer">âž¤</button>
                    </form>
                <?php else: ?>
                    <div style="color: var(--gray); text-align: center; padding: 1rem;">
                        SÃ©lectionnez un coach pour Ã©crire.
                    </div>
                <?php endif; ?>
            </div>
        </div>
...
(function() {
    ...
    const scTheirAvatarUrl = <?= json_encode(!empty($coach['photo_profil']) ? ('/telechargements/profils/' . $coach['photo_profil']) : '') ?>;
    const scTheirAvatarExists = <?= (!empty($coach['photo_profil']) && file_exists(__DIR__ . '/../telechargements/profils/' . ($coach['photo_profil'] ?? ''))) ? 'true' : 'false' ?>;
    const scTheirInitial = <?= json_encode(strtoupper(substr($coach['prenom'] ?? 'C', 0, 1))) ?>;
    const scTheirColor = <?= json_encode(sc_avatar_color(trim(($coach['prenom'] ?? '') . ' ' . ($coach['nom'] ?? '')))) ?>;
    ...
    async function markRead(id) {
        ...
        try {
            ...
            const res = await fetch('/old/api/messages-lus.php', { method: 'POST', body: fd, credentials: 'same-origin' });
            if (!res.ok) return;
    ...
    function appendIncomingMessage(m) {
        ...
        if (scTheirAvatarExists && scTheirAvatarUrl) {
            ...
            fb.textContent = (scTheirInitial || 'C');
            row.appendChild(fb);
        }
    ...
    async function pollChat() {
        ...
        inFlight = true;
        try {
            const res = await fetch('/old/api/actualisation-messages.php?last_id=' + encodeURIComponent(String(lastId)), { credentials: 'same-origin', cache: 'no-store' });
            if (!res.ok) return;
            const data = await res.json();
