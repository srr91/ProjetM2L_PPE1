...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $other_id > 0) {
    ...
    if ($contenu === '') {
        ...
        if ($stmt->execute([$user_id, $other_id, $contenu])) {
            header('Location: /SportConnect/sportif/messages.php?with=' . $other_id);
            exit();
        }
    ...
    <div style="display: grid; grid-template-columns: 320px 1fr; gap: 1.5rem; align-items: start;">
            ...
            <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                ...
                <?php else: ?>
                    <?php foreach ($contacts as $c): ?>
                        ...
                        ?>
                        <a href="/SportConnect/sportif/messages.php?with=<?= (int)$c['id'] ?>"
        ...
        <div class="card" style="min-height: 520px; display: flex; flex-direction: column;">
            <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; gap: 1rem;">
                ...
                <?php if ($coach): ?>
                    <a class="btn btn-outline" href="/SportConnect/pages/profil-coach.php?id=<?= (int)$coach['id'] ?>">Voir profil</a>
                <?php endif; ?>
            </div>
                            ...
                            <div class="sc-msg-row <?= $mine ? 'sc-msg-row--mine' : 'sc-msg-row--theirs' ?>">
                                <?php if (!$mine): ?>
                                    <?php if (!empty($coach['photo_profil']) && file_exists(__DIR__ . '/../telechargements/profils/' . $coach['photo_profil'])): ?>
                                        <img class="sc-avatar" src="/SportConnect/telechargements/profils/<?= htmlspecialchars($coach['photo_profil']) ?>" alt="" />
                                    <?php else: ?>
...
(function() {
    ...
    const scTheirAvatarUrl = <?= json_encode(!empty($coach['photo_profil']) ? ('/SportConnect/telechargements/profils/' . $coach['photo_profil']) : '') ?>;
    ...
    async function markRead(id) {
        ...
        try {
            ...
            const res = await fetch('/SportConnect/api/messages-lus.php', { method: 'POST', body: fd, credentials: 'same-origin' });
            if (!res.ok) return;
