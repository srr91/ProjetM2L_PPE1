...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['supprimer_photo'])) {
        try {
            ...
            $error = "Erreur : Veuillez d'abord exécuter la mise à jour de la base de données. <a href='/utils/executer-update-profils.php' style='color: #FFD700; font-weight: bold;'>Cliquez ici</a>";
        }
    }
    ...
    try {
        ...
        $error = "Erreur : Veuillez d'abord exécuter la mise à jour de la base de données. <a href='/utils/executer-update-profils.php' style='color: #FFD700; font-weight: bold;'>Cliquez ici pour mettre à jour</a>";
    }
}
...
<div class="container" style="max-width: 1000px; margin-top: 2rem;">
    ...
    $bannerStyle = '';
    if ($hasBanner) {
        $bannerStyle = "--sc-banner-url: url(/telechargements/banners/" . rawurlencode($bannerFileSafe) . ");";
    }
    ?>
...
(function() {
    ...
    function applyBannerState(data) {
        ...
        if (file) {
            ...
            banner.style.setProperty('--sc-banner-url', "url('/telechargements/banners/" + file + "')");
        } else {
    ...
    saveBtn.addEventListener('click', async function() {
        ...
        try {
            ...
            const data = await postForm('/old/api/mise-a-jour-banniere.php', fd);
            if (!data.success) {
    ...
    delBtn.addEventListener('click', async function() {
        ...
        try {
            const fd = new FormData();
            const data = await postForm('/old/api/suppression-banniere.php', fd);
            if (!data.success) {
                throw new Error(data.error || 'Erreur');
            }
